...
public class NBTParser {
    ...
    public NBTParser(final String input) {
    ...
    public NBTCompound parse() throws MalformedNBTException {
        ...
        try {
            ...
            if (!reader.canRead()) {
                exception = new MalformedNBTException("The NBT data ended unexpectedly."
                        + "A compound must be closed by a '}' character.", reader.getCursor());
    ...
    private NBTCompound parseCompound(final NBTCompound nbtCompound) throws MalformedNBTException {
        final String key = key();
        reader.skipWhitespace();
        reader.eat(':');
        final NBT value = new NBTValueParser().parse(reader);
