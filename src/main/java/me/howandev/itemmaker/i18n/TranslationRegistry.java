...
public class TranslationRegistry<T> {
    ...
    @Contract("_, _, _ -> this")
    public TranslationRegistry<T> setMessage(final @NotNull Locale locale, final @NotNull Key key, final @NotNull String message) {
        getTranslationTable(locale).setMessage(key, message);
        return this;
    }

    @Contract("_, _ -> this")
    public TranslationRegistry<T> setMessages(final @NotNull Locale locale, final @NotNull Map<Key, String> messages) {
    ...
    public TranslationRegistry<T> addMessageConfiguration(final @NotNull Configuration config, final @NotNull Locale locale) {
        for (String key : config.getKeys(true)) {
            ...
            if (value != null && !value.isEmpty())
                setMessage(locale, Key.of(key), value);
        }
        return this;
    }
     ...
     * <p>4. {@code null} if no text is associated with the specified {@link Key}.
     *
     ...
     * @return The localized text or {@code null} if no text is associated with the specified {@link Key}.
     */
    public @Nullable String getMessage(final @NotNull T context, final @NotNull Key key, final boolean defaultLocaleFallback) {
     ...
     * @return The localized text or {@code null} if no text is associated with the specified {@link Key}.
     */
    public @Nullable String getMessage(final @NotNull T context, final @NotNull Key key) {
